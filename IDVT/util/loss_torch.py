...
#     pos_score = torch.mul(user_emb, pos_item_emb).sum(dim=1)
#     neg_score = torch.mul(user_emb, neg_item_emb).sum(dim=1)
#     loss = nn.functional.softplus(neg_score - pos_score)
#     return torch.mean(loss)
...
def InfoNCE_inter(view1, view2, temperature, b_cos = True):
    if b_cos:
        view1, view2 = F.normalize(view1, dim=1), F.normalize(view2, dim=1)
    pos_score = (view1 * view2).sum(dim=-1)
    pos_score = torch.exp(pos_score / temperature)
...
def InfoNCE_intra(view1 , temperature, b_cos = True):
    if b_cos:
        view1 = F.normalize(view1, dim=1)
    pos_score = (view1 * view1).sum(dim=-1)
    pos_score = torch.exp(pos_score / temperature)
...
def InfoNCE_inter_intra(view1, view2, temperature, b_cos = True):
    if b_cos:
        view1, view2 = F.normalize(view1, dim=1), F.normalize(view2, dim=1)
    pos_score = (view1 * view2).sum(dim=-1)
    pos_score = torch.exp(pos_score / temperature)
    ...
    tt2_score = torch.matmul(view1, view1.transpose(0, 1))
    tt2_score = torch.exp(tt2_score / temperature).sum(dim=1)

    tt_score = ttl_score + tt2_score - pos_score

    cl_loss = -torch.log(pos_score / tt_score + 10e-6)
    return torch.mean(cl_loss)
